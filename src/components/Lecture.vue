...
<template>
  <div
    :style="{ top: `${lectureStartPos}%`, borderColor: '#78938A', height: `${durationHeight}%`, backgroundColor: subject === '午休' ? '#B7D3DF' : subject === '晚餐' ? '#B7D3DF' : '#ffeaea' }"
    w-full absolute text-3 border-rounded-3 flex flex-col justify-center
  >
    <span>{{ subject }}</span>
  </div>
</template>
